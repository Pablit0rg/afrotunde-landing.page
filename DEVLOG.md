...
* **Estado dos Arquivos (v5):** `index.html` (v7), `style.css` (v7), `site.webmanifest` (v1).
...
[X] **Dados (WhatsApp & PIX).**
    * *Plano Concluído:* O número do PIX foi atualizado para o valor real. Os 2 links do WhatsApp (`#whatsapp-btn-hero` e `#whatsapp-btn`) foram atualizados com o número real (`wa.me/5541992929516`) e uma mensagem automática profissional.
...
[X] **UI (Bugs de Estilo).**
    ...
    * *Plano Concluído (Bugfix):* Aumentado o tamanho do `#theme-toggle-mobile` no menu hamburger para `40px` para melhorar a "zona de toque".
...
[ ] **UI (Indicador de Scroll):** (Movido do Banco de Ideias).
    ...
    * **PLANO:** Na próxima sessão, vamos depurar o `script.js` (v4) com calma, garantir que a lógica do `requestAnimationFrame` está correta e que não há erros de sintaxe (como o `});` extra que causei). Estamos atualmente no `script.js` (v3 estável).
...
[ ] **PLACEHOLDERS (REVISÃO FINAL):**
    ...
    * [ ] **Links (WhatsApp/PIX):** (CONCLUÍDO - MOVIDO PARA DONE).
...
[ ] **UI (Efeito Hover na Galeria):** Adicionar `box-shadow` suave e `filter: saturate(1.1)` no hover dos cards da galeria.
